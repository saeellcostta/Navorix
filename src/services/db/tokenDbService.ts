...
import type { TokenListItem, Token, TokenStatus, LaunchMode } from "@/types/token";
...
export type SortOption = "trending" | "new" | "marketcap" | "volume" | "launching";
...
function rowToTokenListItem(
  ...
  // Suporte a stats joinados ou achatados (view tokens_trending)
  ...
  return {
    ...
    // ── Launch Mode fields ──
    status:                 ((r.status as TokenStatus)     ?? "live"),
    launchMode:             ((r.launch_mode as LaunchMode) ?? "instant"),
    escrowSol:              Number(r.escrow_sol              ?? 0),
    graduationThresholdSol: Number(r.graduation_threshold_sol ?? 0.3),
...
export async function getTokens(opts: {
  ...
  if (sort === "launching") {
    ...
    if (error) throw new Error(`getTokens(launching): ${error.message}`);
    return (data ?? []).map((row, i) =>
      rowToTokenListItem(row as TokenRow & { token_stats: TokenStatsRow | null }, offset + i + 1)
    );
  }
